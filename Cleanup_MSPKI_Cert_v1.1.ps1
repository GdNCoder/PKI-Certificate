...
# 
# AUTHOR : Andre Gibel GIBEL .NET / www.gibel.net
# DATE   : 03.11.2014
# Version: 1.1 - 19.12.2014
# 
...
function Remove-ExpiredCertFromDB {
 ...
 #>
    ...
    if ($delete)    
    {
        $CertLogFilePath = "$CleanedFolderLogPath\$pathmid\RequestID-$pathmid-$Date.txt"  
    }
    else
    {
        Write-Host "!!! 'Remove-ExpiredCertFromDB' in 'VIEW MODUS' --> use -delete to delete !!!" -ForegroundColor red -BackgroundColor yellow   
        $CertLogFilePath = "$CleanedFolderLogPath\$pathmid\RequestID-$pathmid-ViewOnly-$Date.txt"  
    }    
